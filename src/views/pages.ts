  ...
  const content = `
    ...
    <div style="margin-bottom: 3.5rem; background: #ffffff; border: 1px solid var(--pink-border); border-radius: var(--radius-xl); padding: clamp(1.5rem, 4vw, 2.5rem); box-shadow: var(--shadow-md);">
      <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem; margin-bottom: 1.25rem;">
        ...
        <div style="display: flex; align-items: center; gap: 0.75rem;">
          <span class="badge badge-emerald" style="padding: 0.4rem 0.85rem; font-size: 0.825rem;">
            🟢 API Status: Active
          </span>
          <a href="/api/upload" class="btn btn-outline btn-sm" style="font-size: 0.825rem; font-weight: 800;">
...
export function getApiUploadPageHtml(): string {
  const content = `
    <div style="max-width: 860px; margin: 0 auto; background: #ffffff; border: 1px solid var(--pink-border); border-radius: var(--radius-xl); padding: clamp(1.75rem, 5vw, 3rem); box-shadow: var(--shadow-lg);">

      <!-- Top Status Banner -->
      <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem; border-bottom: 2px solid var(--pink-border); padding-bottom: 1.25rem; margin-bottom: 2rem;">
        <div>
          <h1 style="font-size: clamp(1.5rem, 3.5vw, 2.25rem); font-weight: 900; color: var(--text-main); letter-spacing: -0.03em;">
            CLI & Terminal Upload API
          </h1>
          <p style="color: var(--text-muted); font-size: 0.95rem; margin-top: 0.25rem;">
            Official documentation & terminal commands for filedontol file sharing.
          </p>
        </div>
        <div style="display: flex; align-items: center; gap: 0.6rem; background: #ecfdf5; border: 1px solid #a7f3d0; padding: 0.6rem 1.15rem; border-radius: 9999px;">
          <span style="width: 10px; height: 10px; background: #10b981; border-radius: 50%; display: inline-block;"></span>
          <span style="color: #047857; font-weight: 900; font-size: 0.9rem;">API Status: Active 🟢</span>
        </div>
      </div>

      <!-- Quick Specs Card Grid -->
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 1.15rem; margin-bottom: 2.25rem;">
        <div style="border: 1px solid var(--pink-border); padding: 1.25rem; border-radius: var(--radius-md); background: var(--pink-soft);">
          <div style="font-size: 0.775rem; font-weight: 900; color: var(--pink-dark); text-transform: uppercase;">MAX FILE SIZE</div>
          <div style="font-size: 1.25rem; font-weight: 900; color: var(--text-main); margin-top: 0.25rem;">5 GB</div>
        </div>
        <div style="border: 1px solid var(--pink-border); padding: 1.25rem; border-radius: var(--radius-md); background: var(--pink-soft);">
          <div style="font-size: 0.775rem; font-weight: 900; color: var(--pink-dark); text-transform: uppercase;">DEFAULT RETENTION</div>
          <div style="font-size: 1.25rem; font-weight: 900; color: var(--text-main); margin-top: 0.25rem;">30 Days</div>
        </div>
        <div style="border: 1px solid var(--pink-border); padding: 1.25rem; border-radius: var(--radius-md); background: var(--pink-soft);">
          <div style="font-size: 0.775rem; font-weight: 900; color: var(--pink-dark); text-transform: uppercase;">AUTO-RESET THRESHOLD</div>
          <div style="font-size: 1.25rem; font-weight: 900; color: var(--text-main); margin-top: 0.25rem;">15 Downloads</div>
        </div>
      </div>

      <!-- Method 1: cURL Multipart Form -->
      <div style="margin-bottom: 2.25rem;">
        <h3 style="font-size: 1.2rem; font-weight: 900; color: var(--text-main); margin-bottom: 0.65rem; display: flex; align-items: center; gap: 0.5rem;">
          <span style="background: var(--pink-deep); color: #ffffff; font-size: 0.8rem; padding: 0.2rem 0.6rem; border-radius: 4px;">POST</span>
          1. Multipart Form Upload (Recommended)
        </h3>
        <p style="color: var(--text-muted); font-size: 0.9rem; margin-bottom: 0.75rem;">
          Upload any file using standard <code>multipart/form-data</code> with cURL:
        </p>
        <div style="background: #111827; border-radius: var(--radius-md); padding: 1.15rem; font-family: monospace; color: #fce7f3; font-size: 0.9rem; overflow-x: auto; border: 1px solid var(--pink-border);">
          <code id="api-curl-form">curl -F "file=@yourfile.zip" https://filedontol.com/api/upload</code>
        </div>
      </div>

      <!-- Method 2: cURL Binary Direct Upload -->
      <div style="margin-bottom: 2.25rem;">
        <h3 style="font-size: 1.2rem; font-weight: 900; color: var(--text-main); margin-bottom: 0.65rem; display: flex; align-items: center; gap: 0.5rem;">
          <span style="background: #8b5cf6; color: #ffffff; font-size: 0.8rem; padding: 0.2rem 0.6rem; border-radius: 4px;">PUT / POST</span>
          2. Binary Direct Upload (-T / --upload-file)
        </h3>
        <p style="color: var(--text-muted); font-size: 0.9rem; margin-bottom: 0.75rem;">
          Stream raw binary file data directly from terminal:
        </p>
        <div style="background: #111827; border-radius: var(--radius-md); padding: 1.15rem; font-family: monospace; color: #fce7f3; font-size: 0.9rem; overflow-x: auto; border: 1px solid var(--pink-border);">
          <code id="api-curl-binary">curl -T "image.png" https://filedontol.com/api/upload</code>
        </div>
      </div>

      <!-- Method 3: PowerShell / Windows -->
      <div style="margin-bottom: 2.5rem;">
        <h3 style="font-size: 1.2rem; font-weight: 900; color: var(--text-main); margin-bottom: 0.65rem;">
          3. Windows PowerShell Example
        </h3>
        <div style="background: #111827; border-radius: var(--radius-md); padding: 1.15rem; font-family: monospace; color: #fce7f3; font-size: 0.875rem; overflow-x: auto; border: 1px solid var(--pink-border);">
          <code id="api-powershell">Invoke-RestMethod -Uri "https://filedontol.com/api/upload" -Method Post -InFile "C:\\path\\file.zip"</code>
        </div>
      </div>

      <!-- Response Format Explanation -->
      <div style="border-top: 1px solid var(--border-color); padding-top: 1.75rem;">
        <h3 style="font-size: 1.15rem; font-weight: 900; color: var(--text-main); margin-bottom: 0.5rem;">Response Formats</h3>
        <ul style="margin-left: 1.25rem; color: var(--text-muted); font-size: 0.9rem; line-height: 1.7;">
          <li><strong>Terminal User-Agents (curl/wget):</strong> Returns plain-text share URL ending with a newline (e.g. <code>https://filedontol.com/f/x8Y2aK1z\n</code>).</li>
          <li><strong>JSON Response (Browsers/Scripts):</strong> Returns <code>{"success": true, "shareCode": "...", "downloadUrl": "..."}</code>.</li>
        </ul>
      </div>

      <div style="margin-top: 2.25rem; text-align: center;">
        <a href="/" class="btn btn-outline">&larr; Return to Home Page</a>
      </div>
    </div>
    ...
    <script>
      document.addEventListener('DOMContentLoaded', function() {
        ...
        const formEl = document.getElementById('api-curl-form');
        const binEl = document.getElementById('api-curl-binary');
        const psEl = document.getElementById('api-powershell');

        if (formEl) formEl.innerText = 'curl -F "file=@yourfile.zip" ' + origin + '/api/upload';
        if (binEl) binEl.innerText = 'curl -T "image.png" ' + origin + '/api/upload';
        if (psEl) psEl.innerText = 'Invoke-RestMethod -Uri "' + origin + '/api/upload" -Method Post -InFile "C:\\\\path\\\\file.zip"';
      });
    </script>
  `;

  return getLayoutHtml('API Upload Status & Documentation', content);
}
